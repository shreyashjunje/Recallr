...
//   // create or reuse a clientAttemptId: keep it stable across refresh while result screen is open
...
//     if (savedAttemptId || isSaving) return; // already saved or in progress
...
//         // compute durationSeconds robustly
//         const startedAt = quizState.startTime ? new Date(quizState.startTime) : null;
//         const completedAt = quizState.completedAt ? new Date(quizState.completedAt) : new Date();
...
//         // Normalize answers if needed - ensure keys are named consistently with backend
//         const answers = (quizState.answers || []).map((a) => ({
//           questionId: a.questionId,
//           selectedAnswer: a.answer ?? a.selectedAnswer ?? null,
//           isCorrect: !!a.isCorrect,
//           timeTaken: typeof a.timeTaken === "number" ? a.timeTaken : a.timeSpent ?? null,
//         }));

//         // Get auth token (adapt to your auth)
...
//         // backend responds with created attempt
//         setSavedAttemptId(resp.data.attempt?._id ?? resp.data.attemptId ?? null);
//       } catch (err) {
...
//     saveQuiz();
//   }, [quizState?.isCompleted, quizState, totalQuestions, savedAttemptId, isSaving, clientAttemptId]);
...
//   const pointsPerQuestion = 100; // keep in sync with Quiz.jsx
...
//   // Determine elapsed seconds robustly
//   const durationSeconds =
...
//   // Format to "M m S s" or show N/A if missing
...
//   // minutes (integer). If you want m:ss, I can adjust.
...
export function QuizResults({ quizState, totalQuestions, onRestart, onHome }) {
  ...
  const performSave = async () => {
    ...
    try {
      ...
      console.log("quizstate->",quizState._id)
      ...
      const attemptId = resp?.data?.attempt?._id ?? resp?.data?.attemptId ?? null;
  ...
  const getGrade = (pct) => {
    ...
    if (pct >= 80) return { grade: "A", color: "text-green-400", bg: "bg-green-500/20" };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex items-center justify-center p-4">
      <div className="max-w-2xl w-full">
        <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl p-8 shadow-2xl text-center">
          ...
          <div className="mb-4">
            {isSaving && <div className="text-sm text-yellow-300">Saving result...</div>}
            {!isSaving && savedAttemptId && <div className="text-sm text-green-300">Saved ✓ (id: {savedAttemptId})</div>}
            {!isSaving && saveError && (
              <div className="text-sm text-red-300 flex items-center justify-center gap-2">
                <span>{saveError}</span>
                <button onClick={handleRetry} className="ml-2 px-3 py-1 bg-red-600 rounded text-white text-xs">Retry</button>
              </div>
            )}
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-6 mb-8">
            ...
            <div className="flex items-center gap-4 p-6 bg-white/5 rounded-2xl">
              ...
              <div className="text-left">
                <p className="text-green-300 text-sm">Correct Answers</p>
                <p className="text-white text-3xl font-bold">{correctAnswers}/{totalQuestions}</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4 p-6 bg-white/5 rounded-2xl">
              ...
              <div className="text-left">
                <p className="text-pink-300 text-sm">Attempted</p>
                <p className="text-white text-3xl font-bold">{attemptedQuestions}/{totalQuestions}</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4 p-6 bg-white/5 rounded-2xl md:col-span-2">
              ...
              <div className="text-left">
                <p className="text-yellow-300 text-sm">Time Taken</p>
                <p className="text-white text-3xl font-bold">{timeTakenDisplay}</p>
              </div>
            </div>
          </div>
